...
def gaussian_copula_reference_beta_lognorm(a, b, mu, sigma, r, n, rng):
    """Reference implementation using numpy/scipy for comparison."""
...
def test_sales_matches_reference_within_error_band():
    ...
    # Convert substratum array to list for quantile calculation
    sales_lib_list = sales_lib.tolist()

    for p in (0.01, 0.05, 0.1, 0.5, 0.9, 0.95, 0.99):
        q_lib = np.quantile(sales_lib_list, p)
        q_ref = np.quantile(sales_ref, p)
        rel_err = abs(q_lib - q_ref) / max(1e-9, abs(q_ref))
        assert rel_err < 0.01, f"Quantile {p}: lib={q_lib:.4f}, ref={q_ref:.4f}, rel_err={rel_err:.4f}"